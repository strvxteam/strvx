...
export default async function DashboardPage() {
  ...
  const [engData, todayGoogleEvents, currentUser, atRiskItems, allTasks] = await Promise.all([
    getPipelineEngagements(),
    teamRefreshToken
  ...
  return (
    <div>
      ...
      <div className="mb-6 grid grid-cols-2 gap-3 lg:grid-cols-4">
        ...
        <div className="rounded-lg border border-[#e0e0e0] bg-white px-4 py-3">
          <p className="text-[10px] font-semibold uppercase tracking-widest text-[#999]">Needs Attention</p>
          <p className={`mt-1 text-[20px] font-bold ${totalActions > 0 ? "text-[#c0392b]" : "text-[#222]"}`}>
            {totalActions}
          </p>
        </div>
        <div className="rounded-lg border border-[#e0e0e0] bg-white px-4 py-3">
        ...
        <div className="rounded-lg border border-[#e0e0e0] bg-white px-4 py-3">
          <p className="text-[10px] font-semibold uppercase tracking-widest text-[#999]">Overdue Tasks</p>
          <p className={`mt-1 text-[20px] font-bold ${overdueTasks.length > 0 ? "text-[#c0392b]" : "text-[#222]"}`}>
            {overdueTasks.length}
          </p>
        </div>
      </div>
