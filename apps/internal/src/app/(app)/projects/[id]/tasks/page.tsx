...
export default async function ProjectTasksTab({
  ...
  if (tasks.length === 0) {
    return <p className="text-[13px] text-[#888]">No tasks for this project.</p>;
  }
  return (
    <div className="divide-y divide-[#f0f0f0] rounded-md border border-[#e0e0e0] bg-white">
      {tasks.map((t) => (
        <div key={t.id} className="flex items-center gap-3 px-4 py-3 text-[13px]">
          <span
            className={`h-2 w-2 rounded-full ${t.status === "done" ? "bg-[#27ae60]" : "bg-[#ccc]"}`}
          />
          <div className="flex-1">
            <p className="text-[#222]">{t.title}</p>
            <p className="text-[11px] text-[#888]">{t.status}</p>
          </div>
        </div>
      ))}
    </div>
