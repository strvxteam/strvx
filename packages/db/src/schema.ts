...
export const interactionTypeEnum = pgEnum("interaction_type", [
  ...
  "action",
  "stage_change",
  // Added for Chief-of-Staff agent (migration 015). Logged when the
  // agent sends or receives an email tied to an engagement.
  "email_sent",
  "email_received",
]);
...
export const companies = pgTable("companies", {
  ...
  // Free-form website (e.g. "acme.com", "https://www.acme.com/about").
  // Used by the Chief-of-Staff CRM hygiene cron's domain_mismatch check;
  // the host is normalized to lowercase + stripped of scheme/www at read
  // time. Added in migration 015 alongside the agent schema.
...
export const interactions = pgTable(
  "interactions",
  {
    ...
    // Chief-of-Staff: when an email_sent/email_received interaction is
    // generated by the agent, this links to the underlying email_messages
    // row. ON DELETE SET NULL so deleting the message doesn't drop the
    // history entry. Forward reference — Drizzle resolves lazily.
    emailMessageId: uuid("email_message_id").references(
      (): AnyPgColumn => emailMessages.id,
      { onDelete: "set null" }
    ),
    createdAt: timestamp("created_at", { withTimezone: true })
      .notNull()
  ...
  (table) => [
    ...
    index("interactions_email_message_idx").on(table.emailMessageId),
  ]
);
...
export const nextActions = pgTable(
  "next_actions",
  {
    ...
    // Chief-of-Staff: when the agent's planner produces a "next action"
    // from a thread (e.g. /extract_actions tool) the row is stamped so
    // the UI can distinguish agent-generated todos from human ones.
...
export const calendarEvents = pgTable("calendar_events", {
  ...
  // Chief-of-Staff: stable cross-calendar id from Google's iCalUID.
  // Used to dedupe when the same event surfaces on multiple connected
  // mailboxes (e.g. meeting on host's + attendee's calendars).
